...
def generate_abstract(text):
    """Gemini model ka istemal karte hue abstract generate karta hai."""
    if not GEMINI_API_AVAILABLE:
        logging.error("Gemini API abstract generation ke liye available nahi. 'google-generativeai' install karein aur API key configure karein.")
        return "Error: Gemini API abstract generation ke liye configure nahi."
    try:
        model = get_gemini_model()
        prompt = (
            f"Darj zail document ya text ke liye mukhtasar aur professional abstract generate karein. "
            f"Abstract mein main objectives, methods, results, aur conclusions ko summarize karna chahiye.\n\n"
            f"Document/Text:\n---\n{text}\n---\n\nAbstract:"
        )
        ...
        if response.candidates and response.candidates[0].content and response.candidates[0].content.parts:
            ...
            # "Abstract:" jaisi shuruati lines ko hata dein
            abstract_text = re.sub(r'^(Abstract:)?\s*', '', abstract_text, flags=re.IGNORECASE).strip()
            return abstract_text
        else:
            return "Gemini se koi abstract generate nahi ho saka."
    except Exception as e:
        logging.error(f"Gemini se abstract generate karne mein error: {e}", exc_info=True)
        return f"Error: Abstract generation fail ho gai. Tafseelat: {str(e)}"