...
def generate_abstract(text):
    """Generates an abstract using the Gemini model."""
    if not GEMINI_API_AVAILABLE:
    ...
    try:
        ...
        prompt = (
            f"Generate a concise and professional abstract in English for the following document or text. " # Explicitly request English
        ...
        logging.error(f"Error generating abstract with Gemini: {e}", exc_info=True)
        return f"Error: Abstract generation failed with Gemini. Details: {str(e)}"