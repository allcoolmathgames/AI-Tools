...
try:
    ...
    if gemini_api_key:
        ...
        logging.warning("GOOGLE_API_KEY environment variable not set. Gemini functions will not work in hook_generator_tool.")
except ImportError:
...
def generate_hooks(topic, tone, target_language="English"): # target_language parameter add kiya
    ...
    if not GEMINI_API_AVAILABLE:
        logging.error(missing_api_key_error_msg("hook_generator_tool"))
        return [missing_api_key_error_msg("hook_generator_tool")]
    
    # Check NLTK data status, though for hook generation, NLTK isn't strictly necessary for the AI part.
    if not _nltk_data_available: # Use the global status flag
        logging.warning("NLTK data setup issue detected, but hook generation might still proceed.")

    try:
        if not _hook_generator_model:
            _hook_generator_model = get_gemini_model()
        
        if not _hook_generator_model:
            return ["Error: Gemini model could not be loaded for hook generation."]
        ...
        response = _hook_generator_model.generate_content(
            prompt,
        ...
        if response.candidates and response.candidates[0].content and response.candidates[0].content.parts:
            ...
            return hooks_list if hooks_list else ["No hooks could be generated."]
        else:
