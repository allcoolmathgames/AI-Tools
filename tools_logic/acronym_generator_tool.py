...
def generate_acronym(text, target_language="English"): # target_language parameter add kiya
    ...
    # Check NLTK data status, though for acronym generation, NLTK isn't strictly necessary for the AI part.
    if not _nltk_data_available: # Use the global status flag
        logging.warning("NLTK data setup issue detected, but acronym generation might still proceed.")

    try:
        if not _acronym_generator_model:
            _acronym_generator_model = get_gemini_model()
        ...
        response = _acronym_generator_model.generate_content(
            prompt,
