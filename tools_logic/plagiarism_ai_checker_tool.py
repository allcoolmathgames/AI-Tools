...
def check_plagiarism_and_ai(text, target_language="English"): # target_language parameter add kiya
    ...
    try:
        ...
        prompt = (
            ...
            f"Provide a percentage score (e.g., '75% AI generated') in {target_language}. " # target_language use kiya
            f"Also, briefly explain your reasoning in {target_language}.\n\n" # target_language use kiya
        ...
        if response.candidates and response.candidates[0].content and response.candidates[0].content.parts:
            ...
            suggestions_from_ai.append(analysis_text) # Use the full analysis as a suggestion

            # Dynamic suggestions based on AI detection result and target language
            if is_ai_generated:
                suggestions_from_ai.append(f"This text appears to be AI-generated. To humanize it, try rephrasing sentences, adding personal anecdotes, and varying sentence structure in {target_language}.") # localized suggestion
            else:
                suggestions_from_ai.append(f"This text appears human-like. No specific suggestions regarding AI detection at this time in {target_language}.") # localized suggestion
            ...
            if plagiarism_percentage > 0:
                suggestions_from_ai.append(f"Approximately {plagiarism_percentage:.2f}% of the text may contain direct matches to existing sources. Please review carefully in {target_language}.") # localized suggestion
            ...
            return {
                ...
                "suggestions": suggestions_from_ai # Use the dynamically generated suggestions
            }

        else:
            return {"is_ai_generated": True, "ai_probability": 0.99, "plagiarism_percentage": 0.0, "suggestions": ["No analysis could be generated by Gemini. Please try different input."]} # Default error localized

    except Exception as e:
        logging.error(f"Error checking AI content with Gemini: {type(e).__name__}: {e}", exc_info=True) # Enhanced error logging
        return {"is_ai_generated": True, "ai_probability": 0.99, "plagiarism_percentage": 0.0, "suggestions": [f"Error: AI content detection failed with Gemini. Details: {str(e)}"]} # Localized error