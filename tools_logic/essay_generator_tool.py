...
def generate_essay(topic, length="medium", style="formal", keywords=""):
    ...
    if not GEMINI_API_AVAILABLE:
        logging.error("Gemini API is not available for essay generation. Please install 'google-generativeai' and configure API key.")
        return "Error: Gemini API not configured for essay generation. Please install 'google-generativeai' and set your API key."
    ...
    try:
        ...
        prompt += "Do not use markdown formatting like **bold** or *italic*. Present the text as plain paragraphs." # Instruction to avoid markdown bold
