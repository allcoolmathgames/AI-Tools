...
try:
    ...
    if gemini_api_key:
        ...
        logging.info("Google Generative AI library loaded and configured for business_name_generator_tool.")
        GEMINI_API_AVAILABLE = True
    else:
        logging.warning("GOOGLE_API_KEY environment variable not set. Gemini functions will not work in business_name_generator_tool.")
except ImportError:
    logging.warning("Google Generative AI library not found. Gemini functions will not work in business_name_generator_tool.")
    GEMINI_API_AVAILABLE = False
except Exception as e:
    logging.error(f"Error configuring Gemini API for business_name_generator_tool: {e}. Gemini functions might not work.")
    GEMINI_API_AVAILABLE = False
...
# --- Business Name Generator Tool Logic ---
...
def generate_business_names(keywords, style, target_language="English"): # target_language parameter add kiya
    ...
    if not GEMINI_API_AVAILABLE:
        logging.error(missing_api_key_error_msg("business_name_generator_tool"))
        return [missing_api_key_error_msg("business_name_generator_tool")]
    
    # Check NLTK data status, though for business name generation, NLTK isn't strictly necessary for the AI part.
    if not _nltk_data_available: # Use the global status flag
        logging.warning("NLTK data setup issue detected, but business name generation might still proceed.")

    try:
        if not _business_name_generator_model:
            _business_name_generator_model = get_gemini_model()
        
        if not _business_name_generator_model:
            return ["Error: Gemini model could not be loaded for business name generation."]
        ...
        response = _business_name_generator_model.generate_content(
            prompt,
        ...
        if response.candidates and response.candidates[0].content and response.candidates[0].content.parts:
            ...
            names_data = json.loads(json_string)
            names_list = [item["name"].strip() for item in names_data if "name" in item]
            return names_list if names_list else ["No business names could be generated."]
        else:
